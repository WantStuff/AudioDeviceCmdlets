...
{
    public abstract class SetAudioDeviceCmdletBase : Cmdlet
    {
        ...
        protected DeviceRoles DeviceRole { get; set; }
        ...
        private void ProcessSwitchInputObject(AudioDevice inputObject)
        {
            ...
            for (int i = 0; i < deviceCollection.Count; i++)
            {
                ...
                if (deviceCollection[i].ID == inputObject.ID)
                {
                    ...
                    client.SetDefaultEndpoint(deviceCollection[i].ID, DeviceRole);
        ...
        private void ProcessIndexSwitch(int index)
        {
            ...
            if (index >= 1 && index <= deviceCollection.Count)
            {
                ...
                // Using PolicyConfigClient, set the given device as the default device (for its type)
                client.SetDefaultEndpoint(deviceCollection[index - 1].ID, DeviceRole);
        ...
        private void ProcessSwitchPlaybackMute(bool recordingMute)
        {
            // Create a new MultiMediaDeviceEnumerator
            var devEnum = new MMDeviceEnumerator();

            // Collect default devices
            var defaultMultimediaPlayback = devEnum.GetDefaultAudioEndpoint(DataFlows.Render, DeviceRole);
        ...
        private void ProcessSwitchPlaybackMuteToggle()
        {
            // Create a new MultiMediaDeviceEnumerator
            var devEnum = new MMDeviceEnumerator();
        ...
        private void ProcessSwitchPlaybackVolume(float playbackVolume)
        {
            // Create a new MultiMediaDeviceEnumerator
            var devEnum = new MMDeviceEnumerator();
        ...
        private void ProcessSwitchRecordingMute(bool recordingMute)
        {
            // Create a new MultiMediaDeviceEnumerator
            var devEnum = new MMDeviceEnumerator();

            // Collect default devices
            var defaultMultimediaRecorder = devEnum.GetDefaultAudioEndpoint(DataFlows.Capture, DeviceRole);
        ...
        private void ProcessSwitchRecordingMuteToggle()
        {
            // Create a new MultiMediaDeviceEnumerator
            var devEnum = new MMDeviceEnumerator();
        ...
        private void ProcessSwitchRecordingVolume(float recordingVolume)
        {
            // Create a new MultiMediaDeviceEnumerator
            var devEnum = new MMDeviceEnumerator();
